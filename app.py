...
def screening_correctives(a,session,limit=3):
    """Select targeted correctives from posture + mobility findings and today's demands."""
    ...
    for _,item,finding,severity in findings:
        ...
        if key in SCREENING_ACTIONS: ids=SCREENING_ACTIONS[key]
            ...
            # Directional shoulder findings are mapped even when the observation
            # is recorded as simply "Right Elevated" / "Left Depressed".
            if any(k in key for k in ["elevated","depressed","scapular"]): ids=SCREENING_ACTIONS.get("scapular asymmetry",["ca10","ca6","ca9"])
            elif "pelvic" in key or "lumbar" in key: ids=["ca4","ca5","ca1"]
            elif "knee" in key or "foot" in key or "ankle" in key: ids=["st3","ca5","ca1"]
            # Exact mobility-joint lookup before generic keyword matching.
            if not ids:
        ...
        if ids:
            ...
            if _v12_side_for_finding(finding)!="B": score+=8
            ...
            for eid in ids: ranked.append((score,eid,_v12_side_for_finding(finding),item))
    # Session-demand correctives remain secondary to actual screening findings.
    if any(k in names for k in ["press","push","bench","handstand"]): ranked.append((25,"ca6","B","session shoulder/scapular preparation"))
    if any(k in names for k in ["squat","lunge","deadlift","rdl","hinge"]): ranked.append((25,"ca5","B","session hip/lower-limb preparation"))
...
def _v12_reference_1rm(a,x):
    n=x.name.lower(); pat=x.pattern.lower()
...
def adaptive_exercise_load(a,x,week,slot="primary"):
    ...
    base=ref*(0.85 if a.primary_goal in ["Max Strength","Strength"] else 0.72 if a.primary_goal=="Hypertrophy" else 0.65)
    last=_v12_last_exercise_log(a,x)
    if not last:
        return _v12_round_load(base), "Initial load from the athlete's recorded strength reference."
    ...
    if actual<=0: return _v12_round_load(base), "No usable previous load recorded."
    ...
    if (rpe<=6.5 and ("excellent" in quality or "good" in quality)) or "easy" in performance:
        inc=5.0 if actual>=60 and any(k in x.name.lower() for k in ["deadlift","squat","rdl","split squat","lunge","hip thrust"]) else 2.5
        return _v12_round_load(actual+inc), "Progressed because the previous exposure was completed with clear reserve and good technique."
...
def build_session(a,week,day,month,rotation,p,system_scores,constraints,history=None,repeat_ids=None):
    ...
    for system in systems:
        if system=="Resistance":
            selector_used=[eid for eid in used if eid not in repeat_ids]
            picks=v7_pick_resistance(a,week,month,2,selector_used,coverage)
            # On a later same-week strength exposure, intentionally re-use one anchor lift.
            if day>=4 and repeat_ids:
                ...
                if anchors:
                    ...
                    picks=[preferred]+[x for x in picks if x.id!=preferred.id]
                    picks=picks[:2]
        elif system=="Neuromuscular Coordination":
...
def render_exercise(a,x,week,constraints,slot):
    if x.system=="Resistance":
        ...
        intensity=f"Planned load {load:g} kg | RPE target {d['rpe']:g}"
...
def save_exercise_feedback(a,session,rows,session_rpe,pain_after,notes):
    ...
    summary={"timestamp":stamp,"athlete":a.name,"sport":a.sport,"month":session["month"],"week":session["week"],"day":session["day"],"session_rpe":float(session_rpe),"pain_after":int(pain_after),"notes":notes}
    st.session_state.session_feedback_log.append(summary)
    st.session_state.last_feedback=summary
    # Keep the legacy summary store alive for existing load modifiers.
    st.session_state.feedback["session_rpe"].append(float(session_rpe)); st.session_state.feedback["pain"].append(int(pain_after))
    return summary
...
if page=="1. Athlete Profile":
    ...
    st.markdown('<div class="banner-header">11 | Session Follow-Up -> Next Session Adaptation</div>',unsafe_allow_html=True)
    ...
    st.markdown(f"<div class='goal-card'><b>Recording: Month {m} | Week {w} | Day {d}</b><br>Enter what actually happened. These records influence the next exposure of each exercise.</div>",unsafe_allow_html=True)
    rows=[]
    for i,x in enumerate(session.get("exercises",[])):
        if x.system!="Resistance":
            continue
        dose=v7_resistance_dose(a,w,"primary" if i<2 else "secondary")
        planned_load,_=adaptive_exercise_load(a,x,w,"primary" if i<2 else "secondary")
        ...
        rows.append({"exercise_id":x.id,"exercise_name":x.name,"planned_load":planned_load,"planned_reps":dose["reps"],"actual_load":actual_load,"reps_completed":reps_done,"exercise_rpe":ex_rpe,"technique":tech,"performance":perf})
    ...
    if st.button("Save Session & Update Next Decisions",type="primary"):
        save_exercise_feedback(a,session,rows,session_rpe,pain_after,notes)
        log_training_session(a,session_rpe,pain_after,performance_change,duration,notes)
        st.success("Session saved. Future exposures will use the recorded loads, reps, RPE, technique, pain and performance.")
    logs=st.session_state.get("exercise_feedback_log",[])
    if logs:
        st.markdown("### Recent Exercise History")
