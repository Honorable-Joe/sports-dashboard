...
# ============================================================
GOALS = [
    "Overall Development", "General Fitness", "Weight Loss", "Weight Gain", "Fat Loss", "Hypertrophy",
...
E = [
    ...
    ex("ca6","Serratus Wall Slide","Corrective / Activation","Scapular",["Stability","Mobility","Scapular Control"],["Bodyweight"],"Beginner",fatigue=1,tags=["Scapular","Serratus","Upward Rotation"]),
    ex("ca7","Prone Y Raise","Corrective / Activation","Scapular",["Stability","Movement Quality","Scapular Control"],["Bodyweight","Bands"],"Beginner",fatigue=1,tags=["Scapular","Lower Trap","Upward Rotation"]),
    ex("ca8","Scapular Push-Up","Corrective / Activation","Scapular",["Stability","Movement Quality","Scapular Control"],["Bodyweight"],"Beginner",fatigue=1,tags=["Scapular","Serratus","Protraction"]),
    ex("ca9","Single-Arm Wall Slide","Corrective / Activation","Scapular",["Stability","Mobility","Scapular Control"],["Bodyweight"],"Beginner",fatigue=1,unilateral=True,tags=["Scapular","Unilateral","Side-Specific"]),
    ex("ca10","Scapular Elevation-Depression Control","Corrective / Activation","Scapular",["Movement Quality","Stability","Scapular Control"],["Bodyweight"],"Beginner",fatigue=1,tags=["Scapular","Elevation","Depression","Control"]),
    ex("ca11","Wall Scapular CAR","Corrective / Activation","Scapular",["Mobility","Movement Quality","Scapular Control"],["Bodyweight"],"Beginner",fatigue=1,tags=["Scapular","Control","Mobility"]),
    # Mobility
...
POSTURE_FINDINGS = {
    ...
    "Posterior": {
        "Scapular position": ["Not assessed", "Neutral / symmetrical", "Bilateral scapular winging", "Right scapular winging", "Left scapular winging", "Bilateral scapular elevation", "Right scapular elevation", "Left scapular elevation", "Bilateral scapular depression", "Right scapular depression", "Left scapular depression", "Bilateral scapular protraction", "Right scapular protraction", "Left scapular protraction", "Scapular asymmetry - other"],
...
# ============================================================
DEFAULTS = {
    "name":"New Athlete","age":25,"sex":"Male","height_cm":180.0,"weight_kg":80.0,"target_weight_kg":80.0,
    ...
    "bench_1rm":70.0,"ohp_1rm":45.0,"deadlift_1rm":120.0,"pullups":8,"pushups":30,"cooper_m":2400.0,"left_jump":105.0,"right_jump":104.0,
...
def multi_choice(label, options, current, key_prefix, columns=2, help_text=None):
    """Touch-friendly multi-select replacement for mobile use.
    Checkboxes stay visible, so selecting several options never requires reopening a dropdown.
    """
    ...
    for i,opt in enumerate(options):
        with cols[i % max(1,columns)]:
            if st.checkbox(str(opt), value=(opt in current), key=f"{key_prefix}_{i}"):
                chosen.append(opt)
    return chosen

def _state_widget_args(args, kwargs, current):
    """Normalize legacy positional Streamlit widget arguments.

    Streamlit's slider/number_input signatures put `value` in the third
    positional slot. Passing that slot positionally AND `value=` by keyword
    raises a TypeError. Convert the common positional form to explicit
    min/max/value/step keywords so state-backed widgets are safe.
    """
    a=list(args)
    kw=dict(kwargs)
    if len(a)>=1 and "min_value" not in kw: kw["min_value"]=a[0]
    if len(a)>=2 and "max_value" not in kw: kw["max_value"]=a[1]
    if len(a)>=3 and "value" not in kw: kw["value"]=a[2]
    if len(a)>=4 and "step" not in kw: kw["step"]=a[3]
    kw["value"]=current
    return kw

def state_slider(label, key, *args, **kwargs):
    """Render a state-backed slider with an independent widget key."""
    if key not in st.session_state:
        if "value" in kwargs: st.session_state[key]=kwargs["value"]
        elif len(args)>=3: st.session_state[key]=args[2]
        elif len(args)>=2: st.session_state[key]=args[1]
    widget_key=f"__aiq_widget_slider_{key}"
    kw=_state_widget_args(args, kwargs, st.session_state[key])
    return st.slider(label, key=widget_key, **kw)

def state_number_input(label, key, *args, **kwargs):
    """Render a state-backed number input with an independent widget key."""
    if key not in st.session_state:
        if "value" in kwargs: st.session_state[key]=kwargs["value"]
        elif len(args)>=3: st.session_state[key]=args[2]
        elif len(args)>=2: st.session_state[key]=args[1]
    widget_key=f"__aiq_widget_number_{key}"
    kw=_state_widget_args(args, kwargs, st.session_state[key])
    return st.number_input(label, key=widget_key, **kw)

def anthropometric_context(a):
    h=max(float(a.height_cm),1.0)/100.0
    bmi_value=float(a.weight_kg)/(h*h)
    if bmi_value < 18.5: category="Below conventional BMI range"
    elif bmi_value < 25: category="Within conventional BMI range"
    elif bmi_value < 30: category="Above conventional BMI range"
    else: category="High BMI range"
    target=float(getattr(a,"target_weight_kg",a.weight_kg) or a.weight_kg)
    delta=target-float(a.weight_kg)
    return {"bmi":bmi_value,"category":category,"target":target,"delta":delta,"height_m":h}

def body_mass_goal_adjustment(a):
    ctx=anthropometric_context(a); out={k:0.0 for k in ["Resistance","Aerobic","Anaerobic / Repeated Sprint","Mobility","Stability / Core"]}
    goal=a.primary_goal
    if goal=="Weight Loss":
        out["Aerobic"]+=24; out["Anaerobic / Repeated Sprint"]+=14; out["Resistance"]+=18
    elif goal=="Weight Gain":
        out["Resistance"]+=28; out["Aerobic"]-=6; out["Anaerobic / Repeated Sprint"]-=5
    elif goal=="Fat Loss":
        out["Aerobic"]+=18; out["Resistance"]+=14
    # Body mass is also a real training constraint: relative strength and
    # bodyweight exercise difficulty use the measured mass, not a display-only BMI.
    if ctx["bmi"] < 18.5: out["Resistance"]+=5
    elif ctx["bmi"] >= 30: out["Mobility"]+=3; out["Stability / Core"]+=3
    return out
    ...
    for view,d in [("Anterior",a.posture_anterior),("Lateral",a.posture_lateral),("Posterior",a.posture_posterior)]:
        for item,val in d.items():
            ...
            if isinstance(val, str) and " || " in val:
                ...
                severity=parts[1].strip() if len(parts)>1 else "Not assessed"
...
def performance_scores(a):
    return {
        ...
        "Relative Squat Strength":float(np.clip(relative_strength(a.squat_1rm,a.weight_kg)/2*100,0,100)),
        "Relative Bench Strength":float(np.clip(relative_strength(a.bench_1rm,a.weight_kg)/1.25*100,0,100)),
        "Height-Normalized CMJ":float(np.clip((a.cmj/max(a.height_cm,1))*100,0,100)),
        "Height-Normalized Broad Jump":float(np.clip((a.broad_jump/max(a.height_cm,1))*50,0,100)),
...
def exercise_allowed(x,a,constraints):
    if "Bodyweight" not in x.equipment and not any(eq in a.equipment for eq in x.equipment): return False
...
def render_session(a,session,week,engine):
    ...
    st.markdown("#### 🔥 MetCon / Energy System Development")
...
SCREENING_RESPONSE_RULES = {
    ...
    "scapular elevation": {"Stability":5,"Mobility":1},
    "scapular depression": {"Stability":5,"Mobility":1},
    "left elevated": {"Stability":4,"Mobility":1},
    "left depressed": {"Stability":4,"Mobility":1},
    "right elevated": {"Stability":4,"Mobility":1},
    "right depressed": {"Stability":4,"Mobility":1},
...
def adaptive_load_modifier(a):
    ...
    comps=st.session_state.get("session_component_feedback_log",[])
    if comps:
        recent=comps[-20:]
        met=[r for r in recent if r.get("component_type")=="MetCon"]
        if met and any(str(r.get("performance","")).lower() in ["hard","failed / could not complete"] for r in met): mod*=.90
        if recent and any(int(r.get("pain",0) or 0)>=5 for r in recent): mod*=.85
...
def log_training_session(a,session_rpe,pain_after,performance_change,duration_min=60,notes=""):
    selected=st.session_state.get("selected_session",{"month":1,"week":1,"day":1})
    rec={"timestamp":datetime.now().isoformat(timespec="seconds"),"athlete":a.name,"sport":a.sport,
         "month":int(selected.get("month",1)),"week":int(selected.get("week",1)),"day":int(selected.get("day",1)),
         "session_rpe":float(session_rpe),"pain_after":int(pain_after),"performance_change":int(performance_change),
         "duration_min":int(duration_min),"notes":notes}
...
AIQ62_GOAL_SYSTEMS = {
    ...
    "Weight Loss": {"Resistance": 20, "Aerobic": 28, "Anaerobic / Repeated Sprint": 16, "Neuromuscular Coordination": 6},
    "Weight Gain": {"Resistance": 34, "Aerobic": 6, "Neuromuscular Coordination": 6},
...
def aiq62_sport_ok(x,a):
    # General Fitness is a non-sport discipline: do not leak Boxing, Karate,
    # Swimming, Tennis, etc. drills into its reaction/agility pool.
    if a.sport=="General Fitness":
        return not x.sport_tags or "General Fitness" in x.sport_tags
    if not aiq62_is_locked(x): return True
    return exercise_sport_compatible(x,a)

def aiq62_equipment_ok(x,a):
    # Bodyweight is an always-available base modality; only external implements
    # are hard-filtered by the athlete's equipment selection.
    if "Bodyweight" in x.equipment: return True
...
def aiq62_select_exercises(a,p,system_scores,system,n,month,used_ids,constraints,coverage=None):
    ...
    used_names={_exercise_identity(EXERCISES[eid]) for eid in used_ids if eid in EXERCISES}
    for x in EXERCISES.values():
        if x.id in used_ids or _exercise_identity(x) in used_names: continue
...
def priorities(a):
    ...
    # Height is used to normalize jumping outputs so taller/shorter athletes are
    # not compared only on raw centimetres. Body mass remains in all relative-strength metrics.
    jump_gap=max(0,100-float(scores.get("Height-Normalized CMJ",50)))
    broad_gap=max(0,100-float(scores.get("Height-Normalized Broad Jump",50)))
    base["Power"] += jump_gap*0.08 + broad_gap*0.04
    ...
    # Height and body mass now affect the decision layer through normalized
    # performance and explicit body-mass goals, rather than being intake-only fields.
    bm=body_mass_goal_adjustment(a)
    for system,delta in bm.items():
        if system=="Resistance": base["Strength"]+=delta
        elif system=="Aerobic": base["Aerobic"]+=delta
        elif system=="Anaerobic / Repeated Sprint": base["Anaerobic"]+=delta
        elif system=="Mobility": base["Mobility"]+=delta
        elif system=="Stability / Core": base["Stability"]+=delta
    flags=screening_flags(a)
    if flags:
...
def system_allocation(a,p,constraints):
    ...
    if a.primary_goal=="Weight Loss":
        out["Resistance"]=out.get("Resistance",0)+22; out["Aerobic"]=out.get("Aerobic",0)+28; out["Anaerobic / Repeated Sprint"]=out.get("Anaerobic / Repeated Sprint",0)+16
    elif a.primary_goal=="Weight Gain":
        out["Resistance"]=out.get("Resistance",0)+34; out["Aerobic"]=out.get("Aerobic",0)*0.65; out["Anaerobic / Repeated Sprint"]=out.get("Anaerobic / Repeated Sprint",0)*0.70
    for g in a.secondary_goals:
...
def session_template(a,day,system_scores,constraints):
    ...
    templates={
        ...
        "Weight Loss":["Corrective / Activation","Resistance","Aerobic","Resistance","Anaerobic / Repeated Sprint"],
        "Weight Gain":["Corrective / Activation","Resistance","Resistance","Neuromuscular Coordination","Aerobic"],
        "Fat Loss":["Corrective / Activation","Resistance","Aerobic","Resistance","Anaerobic / Repeated Sprint"],
        "General Fitness":["Corrective / Activation","Resistance","Resistance","Neuromuscular Coordination","Aerobic"],
        "Overall Development":["Corrective / Activation","Resistance","Resistance","Plyometrics","Aerobic"],
    }
...
def conditioning_decision(a,p,constraints,week,month=1,day=1):
    ...
    return {"system":"Anaerobic / Repeated Sprint" if protocol in ["Tabata","EMOM","AMRAP","Circuit"] else "Aerobic","name":f"{a.sport} | {a.position} | {protocol} | Month {month} Week {week} Day {day}","protocol":protocol,"stations":stations,"work":work,"rest":rest,"intensity":f"Target RPE {target:.1f} | Load modifier x{loadmod:.2f}","reason":f"Sport + position + primary goal + season + readiness + equipment + monthly stimulus selection."}
...
def v7_sport_ok(x,a):
    if a.sport=="General Fitness":
        n=x.name.lower()
        general_forbidden={"Boxing","MMA","Karate","Swimming","Tennis","Racket Sports (Squash/Padel)","Volleyball","Handball"}
        if any(tag in general_forbidden for tag in (x.sport_tags or [])): return False
        if any(term in n for term in ["shadow boxing","shadow kumite","sprawl","counter shadow","boxing reaction","swim-pull","streamline","split step","tennis","racket","karate"]): return False
...
def v7_resistance_dose(a, week, slot):
    ...
    if goal in ["Weight Loss","Fat Loss"]: return {"sets": 2 if week==4 else 3, "reps": 10 if week in [1,4] else 12, "rpe": 7.0 if week in [1,2] else 7.5 if week==3 else 6.0, "rest":"45-90 s", "tempo":"2-0-1-0"}
    if goal=="Weight Gain": return {"sets": 3 if week==4 else 4, "reps": 8 if week in [1,4] else 10, "rpe": 7.0 if week==1 else 7.5 if week in [2,3] else 6.0, "rest":"90-150 s", "tempo":"2-0-1-0"}
...
def v7_score_resistance(x,a,week,month,used,coverage):
    ...
    if a.primary_goal=="Weight Gain" and fam in ["knee_dominant","hip_dominant","horizontal_push","horizontal_pull","vertical_push","vertical_pull"]: score += 20
    if a.primary_goal=="Weight Loss" and fam in ["knee_dominant","hip_dominant","horizontal_push","horizontal_pull","unilateral_lower"]: score += 14
...
def _exercise_identity(x):
    # Treat duplicate database entries with the same displayed exercise name as
    # the same stimulus for diversity purposes.
    return " ".join(x.name.lower().split())

def v7_pick_resistance(a,week,month,n,used,coverage):
    candidates=[]
    used_names={_exercise_identity(EXERCISES[eid]) for eid in used if eid in EXERCISES}
    for x in EXERCISES.values():
        s=v7_score_resistance(x,a,week,month,used,coverage)
        if s>-900: candidates.append((s,x))
    # Penalize/reject repeated displayed exercises, not only duplicate IDs.
    candidates=[(score-80 if _exercise_identity(x) in used_names else score,x) for score,x in candidates]
    candidates.sort(key=lambda z:(z[0], -z[1].fatigue, z[1].id), reverse=True)
    picked=[]
    local=set(used)
    local_names=set(used_names)
    # Pick different families and displayed exercise names whenever possible.
    for _,x in candidates:
        fam=V7_FAMILY[x.id]
        ident=_exercise_identity(x)
        if x.id in local or ident in local_names: continue
        if picked and fam in {V7_FAMILY[y.id] for y in picked} and len(candidates)>n*2: continue
        picked.append(x); local.add(x.id); local_names.add(ident)
...
def session_template(a,day,system_scores,constraints):
    ...
    # Day emphasis changes the conditioning/athletic layer while preserving the
    # required whole-body resistance coverage. This prevents General Fitness from
    # becoming a repeated deadlift-only plan.
    if day%3==2:
        if g in ["Power","Speed","Sport Performance"]: base[4]="Agility / COD"
        if g in ["Endurance","Weight Loss","Fat Loss"]: base[2]="Resistance"
    if day%3==0:
        if g in ["Agility","Sport Performance"]: base[3]="Neuromuscular Coordination"
    return base
...
V7_PROTOCOLS = {
    ...
    "Weight Loss":["AMRAP","Intervals","Circuit","Tabata"],
    "Weight Gain":["Intervals","Circuit","Every 90s","Intervals"],
...
def v7_sport_metcon_pool(a,month,week=1,day=1):
    sport=a.sport
    pos=a.position
    # Rotate the stimulus by month + week + day, not only by month. This prevents
    # the same four stations from repeating across an entire week.
    cycle=(max(1,month)-1)*12 + (max(1,week)-1)*4 + (max(1,day)-1)
    if sport=="Soccer":
        pools=[
            ["5-10 m Acceleration - 3 reps","DB Reverse Lunge - 8/side","Kettlebell Swing - 10 reps","Barbell Push Press - 6 reps"],
            ["Lateral Shuffle to Sprint - 20 s","DB RDL - 8 reps","Kettlebell Goblet Squat - 10 reps","Push-Up - 10 reps"],
            ["Deceleration to Re-Acceleration - 3/side","DB Step-Up - 8/side","Kettlebell Clean - 6/side","Barbell Row - 8 reps"],
            ["Curved Acceleration - 2/side","DB Split Squat - 8/side","Kettlebell Swing - 12 reps","Standing Overhead Press - 8 reps"],
            ["Crossover to Sprint - 2/side","DB Bench Press - 8 reps","Kettlebell Reverse Lunge - 8/side","Barbell RDL - 8 reps"],
            ["Reactive Direction Change - 15 s","DB Single-Arm Row - 8/side","Kettlebell Deadlift - 10 reps","Push-Up - 12 reps"],
        ]
        if pos=="Goalkeeper":
            pools[0]=["Lateral Reaction Shuffle - 15 s","DB Split Squat - 8/side","Kettlebell Swing - 10 reps","Standing Overhead Press - 8 reps"]
            pools[3]=["Lateral Bound to Stick - 4/side","DB Step-Up - 8/side","Kettlebell Clean - 6/side","Barbell Row - 8 reps"]
        elif pos=="Winger":
            pools[1]=["Curved Acceleration - 2/side","DB RDL - 8 reps","Kettlebell Swing - 12 reps","Push-Up - 10 reps"]
            pools[4]=["Crossover to Sprint - 2/side","DB Reverse Lunge - 8/side","Kettlebell Goblet Squat - 10 reps","Barbell Row - 8 reps"]
        return pools[cycle % len(pools)]
    if sport=="Swimming":
        pools=[
            ["DB Pullover - 10 reps","Swimmer Hollow-Body Hold - 30 s","Prone Y-T-W - 6/position","DB Romanian Deadlift - 8 reps"],
            ["DB Bench Press - 8 reps","Dead Bug - 30 s","One-Arm Dumbbell Row - 8/side","Fast March - 30 s"],
            ["DB Overhead Press - 8 reps","Bird Dog - 8/side","DB Reverse Lunge - 8/side","Kettlebell Swing - 10 reps"],
            ["Single-Arm DB Row - 8/side","Hollow-Body Hold - 30 s","DB Split Squat - 8/side","Push-Up - 10 reps"],
        ]
        return pools[cycle % len(pools)]
    if sport in ["Boxing","MMA"]:
        pools=[
            ["Shadow Boxing - 30 s","Reactive Footwork - 20 s","DB Reverse Lunge - 8/side","DB Push Press - 6 reps"],
            ["Visual Reaction Boxing - 20 s","Sprawl to Stand - 5 reps","DB Bench Press - 8 reps","Suitcase Carry - 20 m"],
            ["Reaction Callout - 20 s","Split-Stance Footwork - 20 s","DB Romanian Deadlift - 8 reps","Push-Up - 10 reps"],
            ["Shadow Boxing - 20 s","Lateral Footwork - 20 s","Kettlebell Swing - 10 reps","One-Arm DB Row - 8/side"],
        ]
        return pools[cycle % len(pools)]
    if sport in ["Tennis","Racket Sports (Squash/Padel)"]:
        pools=[
            ["Split Step to Reactive Lateral Shuffle - 15 s","Crossover to 5 m Acceleration - 2/side","DB Split Squat - 8/side","Rotational DB Press - 6/side"],
            ["Visual Reaction COD - 15 s","Lateral Bound to Stick - 4/side","DB RDL - 8 reps","One-Arm Dumbbell Row - 8/side"],
            ["Open-Stance COD - 3/side","Reactive Shuffle - 15 s","Suitcase Carry - 20 m","DB Step-Up - 8/side"],
            ["Crossover Acceleration - 2/side","Lateral Bound - 4/side","DB Bench Press - 8 reps","Kettlebell Swing - 10 reps"],
        ]
        return pools[cycle % len(pools)]
    if sport=="Karate":
        pools=[
            ["Karate Reaction Callout - 20 s","Split-Stance Punch to Lateral Exit - 20 s","DB Reverse Lunge - 8/side","Rotational Medicine-Ball Scoop Toss - 5/side"],
            ["Shadow Kumite - 30 s","Reactive Footwork - 20 s","DB RDL - 8 reps","Suitcase Carry - 20 m"],
            ["Strike-Check-Exit Reaction - 20 s","Lateral Bound to Stick - 4/side","DB Step-Up - 8/side","Cable Rotation - 8/side"],
            ["Kata Direction Flow - 30 s","Split-Stance Footwork - 20 s","Kettlebell Swing - 10 reps","DB Overhead Press - 8 reps"],
        ]
        return pools[cycle % len(pools)]
    if sport=="Volleyball":
        pools=[
            ["Approach Jump - 4 reps","Lateral Shuffle - 20 s","DB Split Squat - 8/side","DB Row - 8/side"],
            ["Block Hop - 4 reps","Reactive Cone Drill - 15 s","DB RDL - 8 reps","DB Shoulder Press - 8 reps"],
            ["Repeated Jump - 5 reps","Lateral Bound to Stick - 4/side","DB Step-Up - 8/side","Cable Row - 8 reps"],
            ["Approach Acceleration - 3 reps","Pogo Hops - 10 reps","Kettlebell Swing - 10 reps","Push-Up - 10 reps"],
        ]
        return pools[cycle % len(pools)]
    # General Fitness: also rotate whole-body stations rather than repeating a fixed circuit.
    pools=[
        ["Fast March / Low-Impact Run - 30 s","DB Goblet Squat - 8 reps","Push-Up - 10 reps","One-Arm Dumbbell Row - 8/side"],
        ["Tempo Shuttle - 30 s","DB Reverse Lunge - 8/side","DB Bench Press - 8 reps","Dead Bug - 30 s"],
        ["Lateral Shuffle - 20 s","DB RDL - 8 reps","DB Shoulder Press - 8 reps","Suitcase Carry - 20 m"],
        ["Step-Up - 8/side","Kettlebell Swing - 12 reps","Push-Up - 12 reps","Single-Leg Balance Reach - 6/side"],
        ["Marching High Knees - 30 s","Barbell Row - 8 reps","Kettlebell Goblet Squat - 10 reps","Dead Bug - 30 s"],
        ["Low-Impact Shuttle - 30 s","DB Split Squat - 8/side","Standing Overhead Press - 8 reps","One-Arm Dumbbell Row - 8/side"],
    ]
    return pools[cycle % len(pools)]
...
def conditioning_decision(a,p,constraints,week,month=1,day=1):
    ...
    stations=[s for s in v7_sport_metcon_pool(a,month,week,day) if v7_equipment_token_ok(a,s)]
    ...
    for view,d in [("Anterior",a.posture_anterior),("Lateral",a.posture_lateral),("Posterior",a.posture_posterior)]:
        for item,val in d.items():
            if isinstance(val,str) and " || " in val:
                ...
                severity=parts[1].strip() if len(parts)>1 else "Not assessed"
                if finding not in ("Not assessed","Neutral","Level") and severity not in ("Not assessed","None / neutral"):
                    findings.append((view,item,finding.lower(),severity.lower()))
    return findings

def screening_correctives(a,session,limit=2):
    """Concrete low-fatigue correctives selected from screening findings and today's movement demands."""
    findings=_screening_findings(a)
    text=" ".join(x.name.lower() for x in session.get("exercises",[]))
    specific=[]
    shoulder_issue=any(any(k in f[2] for k in ["scapular","shoulder","left elevated","left depressed","right elevated","right depressed"]) for f in findings)
    hip_issue=any(any(k in f[2] for k in ["pelvic","hip","lumbar"]) for f in findings)
    for view,item,finding,severity in findings:
        if "scapular depression" in finding or "scapular elevation" in finding or "scapular asymmetry" in finding:
            specific += ["ca10","ca6","ca7","ca9"]
        elif "scapular winging" in finding:
            specific += ["ca6","ca8","ca9"]
        elif "scapular protraction" in finding or "rounded" in finding:
            specific += ["ca8","ca6","ca7"]
        elif any(k in finding for k in ["left elevated","left depressed","right elevated","right depressed"]):
            specific += ["ca10","ca9","ca6"]
        elif "pelvic" in finding or "lumbar" in finding:
            specific += ["ca4","ca5","ca1"]
        elif "knee" in finding:
            specific += ["st3","ca5","ca1"]
    if shoulder_issue and any(k in text for k in ["press","push","bench","row","pull","pulldown","handstand"]):
        specific += ["ca6","ca8","ca9"]
    if hip_issue and any(k in text for k in ["squat","lunge","deadlift","rdl","hinge","hip thrust","jump","sprint"]):
        specific += ["ca4","ca5","ca1"]
    if any(k in text for k in ["agility","reactive","shuffle","sprint","jump","bound"]):
        specific += ["st3","ca5"]
    specific += ["ca6","ca5","ca1"]
    out=[]
    for eid in specific:
        x=EXERCISES.get(eid)
        if not x or x.id in out or x.system!="Corrective / Activation": continue
        if not aiq62_equipment_ok(x,a) or not aiq62_level_ok(x,a): continue
        out.append(x.id)
        if len(out)>=limit: break
    return [EXERCISES[eid] for eid in out]

def smart_warmup(a,session):
    systems=set(session.get("systems",[]))
    names=" ".join(x.name.lower() for x in session.get("exercises",[]))
    out=[]
    def add(name,prescription):
        item=f"{name} — {prescription}"
        if item not in out: out.append(item)
    if "Ergometers (AirBike/Rower/SkiErg)" in a.equipment:
        add("AirBike / Rower / SkiErg", "3-5 min easy, RPE 3-4")
    else:
        add("Fast March / Easy Jog", "3-5 min easy")
    if any(k in names for k in ["squat","split squat","lunge","step-up","jump"]):
        add("Squat-to-Stand", "1 x 6 reps")
        add("Ankle Rock + Knee Drive", "1 x 6/side")
    if any(k in names for k in ["deadlift","rdl","hinge","hip thrust"]):
        add("Hip Hinge Drill", "2 x 6 reps")
        add("Glute Bridge", "1 x 8 reps with 2 s squeeze")
    if any(k in names for k in ["press","push","bench","handstand"]):
        add("Scapular Push-Up", "1-2 x 8 reps")
        add("Serratus Wall Slide", "1-2 x 6 reps")
    elif any(k in names for k in ["row","pull","pulldown"]):
        if "Bands" in a.equipment:
            add("Band Pull-Apart", "1-2 x 10-12 reps")
        else:
            add("Wall Scapular CAR", "1 x 5/side")
    if "Plyometrics" in systems or "Acceleration / Speed" in systems or any(k in names for k in ["jump","sprint","bound","power"]):
        add("Pogo Hops", "2 x 5 contacts")
        add("Build-Up Run", "2 x 10-20 m progressive exposures")
    elif "Agility / COD" in systems or any(k in names for k in ["reactive","shuffle","cod"]):
        add("Lateral Shuffle", "2 x 5 m/side")
        add("Deceleration Stick", "2 x 2/side")
    if "Neuromuscular Coordination" in systems:
        add("Single-Leg Balance Reach", "2 x 3/side")
        add("Visual / Verbal Reaction Callout", "2 x 10 s, low fatigue")
    findings=_screening_findings(a)
    ftext=" ".join(f[2] for f in findings)
    if any(k in ftext for k in ["pelvic","lumbar","hip"]):
        add("90/90 Hip Flow", "1 x 5/side")
        add("Dead Bug", "1 x 5/side, controlled")
    if any(k in ftext for k in ["scapular","shoulder","left elevated","left depressed","right elevated","right depressed"]):
        add("Serratus Wall Slide", "2 x 6 reps, smooth scapular control")
        add("Prone Y Raise", "1-2 x 6-8 reps, light and controlled")
    return out[:9]
...
def render_session(a,session,week,engine):
    ...
    correctives=screening_correctives(a,session,limit=2)
    st.markdown("#### 🛠️ Screening Correctives / Movement Control")
    for x in correctives:
        render_exercise(a,x,week,c,"accessory")
    st.caption("Correctives are selected from the recorded screening findings and today's movement demands. Use controlled, pain-free execution; this is a coaching screen, not a diagnosis.")
    for i,x in enumerate(session["exercises"]):
        if x.system=="Corrective / Activation": continue
        slot="primary" if i<2 else "secondary"
        render_exercise(a,x,week,c,slot)
    if session.get("complex"):
        cx=session["complex"]; rounds,reps,between,round_rest=complex_dose(cx,a,week,c)
    ...
    render_card("5. Dynamic MetCon / ESD Protocol",cond["name"],cond["work"]+" | Rest: "+cond["rest"],cond["intensity"],"Dynamic Pace","Multi-planar","Energy System / Conditioning",ACCENTS["Aerobic"])
    st.markdown("**Exact execution:**")
    for station in cond["stations"]: st.markdown(f"- **{station}**")
    st.caption(cond["reason"])

# ============================================================

# ============================================================
# ATHLETE-IQ V12 - SCREENING + ASYMMETRY + SESSION ADAPTATION
# ============================================================
# V12 adds a true session-to-session feedback loop while preserving
# the V11 sport/equipment/diversity architecture.

# ---- richer screening -> action map ---------------------------------
SCREENING_ACTIONS = {
    "anterior pelvic tilt": ["ca4", "ca5", "ca1"],
    "posterior pelvic tilt": ["ca4", "ca1"],
    "pelvic hike": ["ca5", "ca4", "ca1"],
    "scapular winging": ["ca6", "ca8", "ca9"],
    "right scapular winging": ["ca6", "ca8", "ca9"],
    "left scapular winging": ["ca6", "ca8", "ca9"],
    "scapular asymmetry": ["ca10", "ca6", "ca9"],
    "right scapular elevation": ["ca10", "ca6", "ca7"],
    "left scapular elevation": ["ca10", "ca6", "ca7"],
    "scapular elevation": ["ca10", "ca6", "ca7"],
    "right scapular depression": ["ca10", "ca6", "ca9"],
    "left scapular depression": ["ca10", "ca6", "ca9"],
    "scapular depression": ["ca10", "ca6", "ca9"],
    "right scapular protraction": ["ca8", "ca6", "ca7"],
    "left scapular protraction": ["ca8", "ca6", "ca7"],
    "scapular protraction": ["ca8", "ca6", "ca7"],
    "rounded shoulders": ["ca8", "ca6", "ca7"],
    "forward head": ["ca7", "ca9"],
    "thoracic kyphosis": ["ca7", "ca8", "ca9"],
    "foot pronation": ["st3", "ca5", "ca1"],
    "knee valgus": ["st3", "ca5", "ca1"],
    "knee varus": ["st3", "ca5"],
}

# Reference ranges are coaching-screen reference targets, not diagnoses.
# Each entry is (low, high) in degrees. Where a universal range is not
# appropriate, the engine uses side-to-side difference as the stronger signal.
MOBILITY_REFERENCE = {
    "Cervical Flexion": (45, 60), "Cervical Extension": (50, 70), "Cervical Rotation": (60, 80),
    "Shoulder Flexion": (160, 180), "Shoulder Abduction": (160, 180),
    "Shoulder IR": (50, 70), "Shoulder ER": (80, 100),
    "Thoracic Rotation": (40, 60),
    "Hip Flexion": (110, 130), "Hip Extension": (10, 20), "Hip Abduction": (35, 50),
    "Hip Adduction": (20, 35), "Hip IR": (30, 45), "Hip ER": (40, 55),
    "Knee Flexion": (125, 145), "Ankle Dorsiflexion": (30, 45),
    "Ankle Plantarflexion": (45, 60), "Ankle Inversion": (25, 40), "Ankle Eversion": (10, 20),
    "Wrist Flexion": (70, 90), "Wrist Extension": (60, 80),
}
MOBILITY_CORRECTIVES = {
    "Cervical Flexion": ["ca7"], "Cervical Extension": ["ca7"], "Cervical Rotation": ["ca7"],
    "Shoulder Flexion": ["ca6", "ca7"], "Shoulder Abduction": ["ca6", "ca7"],
    "Shoulder IR": ["ca7", "ca8"], "Shoulder ER": ["ca6", "ca9"],
    "Thoracic Rotation": ["ca7", "ca9"],
    "Hip Flexion": ["ca4", "ca1"], "Hip Extension": ["ca4", "ca5"],
    "Hip Abduction": ["ca5", "ca4"], "Hip Adduction": ["ca4", "ca5"],
    "Hip IR": ["ca4", "ca5"], "Hip ER": ["ca4", "ca5"],
    "Knee Flexion": ["ca5", "ca1"], "Ankle Dorsiflexion": ["ca5", "ca1"],
    "Ankle Plantarflexion": ["ca5"], "Ankle Inversion": ["ca5"], "Ankle Eversion": ["ca5"],
    "Wrist Flexion": ["ca7"], "Wrist Extension": ["ca7"],
}

# Exercise-history storage is per athlete and exercise. It survives reruns in
# the current Streamlit session and is used by the next program decision.
if "exercise_feedback_log" not in st.session_state:
    st.session_state.exercise_feedback_log=[]
if "session_feedback_log" not in st.session_state:
    st.session_state.session_feedback_log=[]
if "session_component_feedback_log" not in st.session_state:
    st.session_state.session_component_feedback_log=[]
if "selected_session" not in st.session_state:
    st.session_state.selected_session={"month":1,"week":1,"day":1}


def _v12_findings(a):
    findings=list(_screening_findings(a))
    # Mobility findings are added only when there is a meaningful low-range or
    # side-to-side signal. This avoids turning tiny measurement noise into a flag.
    for joint,pair in (a.mobility_rom or {}).items():
        if not isinstance(pair,dict) or "L" not in pair or "R" not in pair:
            continue
        try:
            L=float(pair["L"]); R=float(pair["R"])
        except Exception:
            continue
        ref=MOBILITY_REFERENCE.get(joint)
        lowL=bool(ref and L < ref[0]); lowR=bool(ref and R < ref[0])
        diff=abs(L-R)
        if lowL or lowR or diff>=8:
            side="Left" if lowL and not lowR else "Right" if lowR and not lowL else "Both" if lowL and lowR else "Asymmetry"
            findings.append(("Mobility",joint,joint.lower(),side))
    return findings


def _v12_side_for_finding(finding):
    f=finding.lower()
    if "left" in f and "right" not in f: return "L"
    if "right" in f and "left" not in f: return "R"
    return "B"


def screening_correctives(a,session,limit=3):
    """Select varied, side-aware correctives from screening + today's demands.
    Rotation changes across week/day so one corrective is not repeated for every session.
    """
    findings=_v12_findings(a)
    names=" ".join(x.name.lower() for x in session.get("exercises",[]))
    day=int(session.get("day",1)); week=int(session.get("week",1)); month=int(session.get("month",1))
    ranked=[]
    for _,item,finding,severity in findings:
        key=finding.lower()
        ids=[]
        if key in SCREENING_ACTIONS: ids=SCREENING_ACTIONS[key]
        elif "scapular" in key or "elevated" in key or "depressed" in key:
            if "depression" in key: ids=["ca10","ca9","ca6"]
            elif "elevation" in key: ids=["ca10","ca6","ca7"]
            elif "winging" in key: ids=["ca6","ca8","ca9"]
            else: ids=["ca10","ca6","ca9"]
        elif "pelvic" in key or "lumbar" in key: ids=["ca4","ca5","ca1"]
        elif "knee" in key or "foot" in key or "ankle" in key: ids=["st3","ca5","ca1"]
        if not ids:
            for joint,ids0 in MOBILITY_CORRECTIVES.items():
                if key==joint.lower(): ids=ids0; break
        if ids:
            side=_v12_side_for_finding(finding)
            score=100 + (20 if severity in ["marked","right","left"] else 0)
            if side!="B": score+=10
            if any(k in names for k in ["press","push","bench","handstand","row","pull","pulldown"]) and any(k in key for k in ["scapular","shoulder","thoracic"]): score+=18
            if any(k in names for k in ["squat","lunge","deadlift","rdl","hinge","jump","sprint"]) and any(k in key for k in ["pelvic","hip","ankle","knee"]): score+=18
            # Rotate equally relevant correctives across the week.
            offset=(day-1)+(week-1)*2+(month-1)
            ids=ids[offset % len(ids):]+ids[:offset % len(ids)] if ids else ids
            for rank,eid in enumerate(ids): ranked.append((score-rank*2,eid,side,item))
    # Session-demand correctives are a secondary pool.
    demand=[]
    if any(k in names for k in ["press","push","bench","handstand","row","pull"]): demand += ["ca6","ca8","ca9","ca10"]
    if any(k in names for k in ["squat","lunge","deadlift","rdl","hinge","jump","sprint"]): demand += ["ca5","ca4","ca1","st3"]
    if any(k in names for k in ["agility","reactive","shuffle","sprint","jump","bound"]): demand += ["st3","ca5","ca10"]
    offset=(day-1)+(week-1)*2+(month-1)
    demand=demand[offset%len(demand):]+demand[:offset%len(demand)] if demand else []
    for i,eid in enumerate(demand): ranked.append((35-i,eid,"B","session demand"))
    ranked.sort(key=lambda z:(z[0],z[1]),reverse=True)
    out=[]; seen=set()
    for _,eid,side,reason in ranked:
        x=EXERCISES.get(eid)
        if not x or eid in seen or x.system!="Corrective / Activation": continue
        if not aiq62_equipment_ok(x,a) or not aiq62_level_ok(x,a): continue
        out.append((x,side,reason)); seen.add(eid)
        if len(out)>=limit: break
    return out


def _v12_last_exercise_log(a,x):
    logs=[r for r in st.session_state.get("exercise_feedback_log",[]) if r.get("athlete")==a.name and r.get("exercise_id")==x.id]
    return logs[-1] if logs else None


def _v12_reference_1rm(a,x):
    n=x.name.lower()
    if any(k in n for k in ["bench press","chest press","push press"]): return float(getattr(a,"bench_1rm",0) or 0)
    if any(k in n for k in ["shoulder press","overhead press"]): return float(getattr(a,"ohp_1rm",0) or 0)
    if any(k in n for k in ["deadlift","rdl","romanian"]):
        d=float(getattr(a,"deadlift_1rm",0) or 0)
        return d if d>0 else float(getattr(a,"squat_1rm",0) or 0)*1.15
    if any(k in n for k in ["squat","split squat","lunge","step-up"]): return float(getattr(a,"squat_1rm",0) or 0)
    if "row" in n or "pulldown" in n or "pull-up" in n: return float(getattr(a,"bench_1rm",0) or 0)*0.65
    return 0.0

def _v12_round_load(load):
    if load<=0: return 0.0
    step=2.5 if load>=50 else 1.25
    return round(load/step)*step

def _v13_load_mode(x):
    """Return (mode, reference multiplier) for practical implement loading.
    Dumbbell bilateral lifts are displayed as total pair load + per-hand load,
    so a 112 kg barbell-style number is never presented as one DB in each hand.
    """
    n=x.name.lower()
    if "dumbbell" in n or n.startswith("db "):
        if any(k in n for k in ["rdl","romanian","deadlift","squat","lunge","step-up","split squat"]):
            return "pair_total",0.55
        if any(k in n for k in ["bench","press","row","pullover","curl"]):
            return "pair_total",0.55
        return "pair_total",0.50
    if "kettlebell" in n or n.startswith("kb "):
        return "single",0.40
    if "barbell" in n or "back squat" in n or "bench press" in n or "deadlift" in n or "rdl" in n:
        return "barbell",1.0
    return "single",1.0

def _v13_load_from_reference(a,x,goal):
    ref=_v12_reference_1rm(a,x)
    if ref<=0: return 0.0
    base_pct=0.85 if goal in ["Max Strength","Strength"] else 0.72 if goal in ["Hypertrophy","Weight Gain"] else 0.65
    mode,mult=_v13_load_mode(x)
    return ref*base_pct*mult

def _v13_load_display(x,load):
    mode,_=_v13_load_mode(x)
    if load<=0: return "Bodyweight / self-selected load"
    if mode=="pair_total": return f"Planned pair load {load:g} kg total ({load/2:g} kg/hand)"
    if mode=="single": return f"Planned load {load:g} kg"
    return f"Planned barbell load {load:g} kg"

def adaptive_exercise_load(a,x,week,slot="primary"):
    dose=v7_resistance_dose(a,week,slot)
    base=_v13_load_from_reference(a,x,a.primary_goal)
    last=_v12_last_exercise_log(a,x)
    if not last:
        return _v12_round_load(base), "Initial load scaled to the exercise implement and the athlete's recorded strength reference."
    actual=float(last.get("actual_load",0) or 0)
    rpe=float(last.get("exercise_rpe",8) or 8)
    completed=float(last.get("reps_completed",0) or 0); planned=float(last.get("planned_reps",dose["reps"]) or dose["reps"])
    quality=str(last.get("technique","" )).lower()
    performance=str(last.get("performance","" )).lower()
    pain=float(last.get("pain",0) or 0)
    if actual<=0: return _v12_round_load(base), "No usable previous load recorded; using the implement-scaled reference."
    if pain>=5 or "poor" in quality or completed < planned:
        new=actual*(0.95 if pain<5 else 0.90)
        return _v12_round_load(new), "Reduced after pain, missed work, or technical breakdown."
    if (rpe<=6.5 and ("excellent" in quality or "good" in quality)) or "easy" in performance:
        step=5.0 if actual>=60 and any(k in x.name.lower() for k in ["deadlift","squat","rdl","split squat","lunge","hip thrust"]) else 2.5
        return _v12_round_load(actual+step), "Progressed because the previous exposure was completed with clear reserve and good technique."
    if rpe<=8.0 and completed>=planned:
        return _v12_round_load(actual+(2.5 if actual>=60 else 1.25)), "Small progression after successful completion at an appropriate effort."
    return _v12_round_load(actual), "Load maintained because the previous exposure was appropriately challenging."

def _v12_anchor(x):
    n=x.name.lower()
    return any(k in n for k in ["deadlift","squat","bench press","shoulder press","row","pull-up","hip thrust","romanian"])

# Permit key lifts to reappear later in the same week. General diversity remains
# active for accessories and conditioning.
def _v13_fill_resistance(a,week,month,want,used,coverage,blocked_families=None):
    if want<=0: return []
    blocked_families=set(blocked_families or [])
    used_names={_exercise_identity(EXERCISES[eid]) for eid in used if eid in EXERCISES}
    c=constraint_engine(a); candidates=[]
    for x in EXERCISES.values():
        ident=_exercise_identity(x)
        if x.system!="Resistance" or x.id in used or ident in used_names: continue
        if V7_FAMILY.get(x.id,v7_family(x)) in blocked_families: continue
        if not aiq62_exercise_allowed(x,a,c): continue
        score=v7_score_resistance(x,a,week,month,used,coverage)
        if score<=-900: continue
        # Relax the family-coverage penalty only for filling a missing slot.
        score += 10 if V7_FAMILY.get(x.id,v7_family(x)) not in coverage else 0
        candidates.append((score,x))
    candidates.sort(key=lambda z:(z[0],-z[1].fatigue,z[1].id),reverse=True)
    out=[]; local_names=set(used_names); local_ids=set(used)
    for _,x in candidates:
        ident=_exercise_identity(x)
        if ident in local_names or x.id in local_ids: continue
        out.append(x); local_names.add(ident); local_ids.add(x.id)
        if len(out)>=want: break
    return out

def build_session(a,week,day,month,rotation,p,system_scores,constraints,history=None,repeat_ids=None):
    systems=session_template(a,day,system_scores,constraints)
    used=list(history or [])[-48:]; repeat_ids=set(repeat_ids or [])
    exercises=[]; coverage=set()
    for system in systems:
        if system=="Resistance":
            # Day 2/3 respect recent-history diversity. Only the later same-week
            # re-exposure (Day 4+) may bring back one Day-1 anchor.
            selector_used=[eid for eid in used if not (day>=4 and eid in repeat_ids)]
            picks=v7_pick_resistance(a,week,month,2,selector_used,coverage)
            if day>=4 and repeat_ids:
                anchors=[EXERCISES[eid] for eid in repeat_ids if eid in EXERCISES and EXERCISES[eid].system=="Resistance"]
                if anchors:
                    preferred=sorted(anchors,key=lambda x:(x.fatigue,x.id))[0]
                    # Never allow the second pick to be another Day-1 anchor.
                    picks=[preferred]+[x for x in picks if x.id not in repeat_ids and x.id!=preferred.id and V7_FAMILY.get(x.id,v7_family(x))!=V7_FAMILY.get(preferred.id,v7_family(preferred))]
                    picks=picks[:2]
                    if len(picks)<2:
                        already=used+[x.id for x in picks]
                        picks += _v13_fill_resistance(a,week,month,2-len(picks),already,coverage,{V7_FAMILY.get(preferred.id,v7_family(preferred))})
            # Never leave a resistance slot empty when the equipment and readiness
            # constraints permit another valid exercise.
            if len(picks)<2:
                already=used+[x.id for x in picks]
                picks += _v13_fill_resistance(a,week,month,2-len(picks),already,coverage)
        elif system=="Neuromuscular Coordination":
            x=v7_select_neuromuscular(a,week,month,used); picks=[x] if x else []
        else:
            picks=aiq62_select_exercises(a,p,system_scores,system,1,month,used,constraints,coverage)
        for x in picks:
            if x and x.id not in [z.id for z in exercises]:
                exercises.append(x); _aiq62_coverage_add(coverage,x)
                if x.id not in repeat_ids: used.append(x.id)
    # Final whole-session safeguard: if the architecture contains two resistance
    # slots, fill both slots even when the family selector exhausted one slot.
    resistance_needed=systems.count("Resistance")*2
    resistance_have=[x for x in exercises if x.system=="Resistance"]
    if len(resistance_have)<resistance_needed:
        fill_used=used+[x.id for x in exercises if x.id not in used]
        extra=_v13_fill_resistance(a,week,month,resistance_needed-len(resistance_have),fill_used,coverage)
        for x in extra:
            exercises.append(x); _aiq62_coverage_add(coverage,x)
    cond=conditioning_decision(a,p,constraints,week,month,day)
    complex_block=None
...
def build_program(a,months):
    constraints=constraint_engine(a); p=priorities(a); systems=system_allocation(a,p,constraints)
    rotation=build_rotation(a,months,p,systems,constraints)
    days=max(1,min(a.gym_days_available,4)); program={}; history=[]
    for m in range(1,months+1):
        program[m]={}
        for w in range(1,5):
            program[m][w]=[]; week_anchors=set()
            for d in range(1,days+1):
                s=build_session(a,w,d,m,rotation,p,systems,constraints,history,week_anchors)
                program[m][w].append(s)
                if d==1:
                    week_anchors.update(x.id for x in s.get("exercises",[]) if _v12_anchor(x))
                history.extend(x.id for x in s.get("exercises",[]))
                if s.get("complex"): history.extend(eid for eid in s["complex"].exercises if eid in EXERCISES)
                history=history[-48:]
    return program,{"constraints":constraints,"priorities":p,"systems":systems,"rotation":rotation,"load_status":constraints.get("load_status"),"movement_engine":"V12 Screening + Adaptive Session Loop","coverage":"full-body resistance + sport/position + goal + screening + unilateral/bilateral + neuromuscular + session feedback"}


def render_exercise(a,x,week,constraints,slot):
    if x.system=="Resistance":
        d=v7_resistance_dose(a,week,slot); load,load_reason=adaptive_exercise_load(a,x,week,slot)
        intensity=f"{_v13_load_display(x,load)} | RPE target {d['rpe']:g}"
        tempo=x.tempo_hypertrophy if a.primary_goal=="Hypertrophy" else x.tempo_power if a.primary_goal in ["Power","Speed"] else x.tempo_strength
        pres=f"{d['sets']} Sets x {d['reps']} Reps | Rest {d['rest']}"
    elif x.system=="Plyometrics":
        pres,rest,intensity=plyo_dose(a,week,constraints); tempo=f"Explosive | {rest}"; load_reason=""
    elif x.system=="Agility / COD":
        pres,rest,intensity=agility_dose(a,week,constraints); tempo=f"Full recovery | {rest}"; load_reason=""
    elif x.system=="Acceleration / Speed":
        pres,rest,intensity=speed_dose(a,week,constraints); tempo=f"Full recovery | {rest}"; load_reason=""
    elif x.system=="Mobility": pres="2 Sets x 6-10 controlled reps/side"; intensity="Controlled ROM"; tempo="2-1-2-0"; load_reason=""
    elif x.system in ["Stability / Core","Corrective / Activation"]: pres="2-3 Sets x 8-12 reps or 20-40 s"; intensity="RPE 5-7"; tempo="Controlled"; load_reason=""
    else: pres="2-3 Sets x 8-12"; intensity="RPE 6-7"; tempo="2-1-2-0"; load_reason=""
    render_card(x.system,x.name,pres,intensity,tempo,x.plane,x.tier,ACCENTS.get(x.system,"#38bdf8"))
    if load_reason: st.caption(load_reason)
    if x.regression or x.progression: st.caption(f"Regression: {x.regression or '-'} | Progression: {x.progression or '-'}")
    st.caption(progression_rule(a,week,x,slot,constraints))
...
def render_session(a,session,week,engine):
    ...
    render_card("1. Smart Warm-up","Session-specific preparation","Low fatigue | 5-12 min","Prepare, do not exhaust","Progressive","Multi-planar","RAMP-style preparation",ACCENTS["Corrective / Activation"])
    for item in warm: st.markdown(f"- {item}")
    correctives=screening_correctives(a,session,limit=3)
    st.markdown("#### 2. Screening Correctives / Asymmetry Control")
    if correctives:
        for x,side,reason in correctives:
            render_exercise(a,x,week,c,"accessory")
            st.caption(f"Screening link: {reason} | Side emphasis: {'Left' if side=='L' else 'Right' if side=='R' else 'Both'}")
    else: st.caption("No targeted corrective was triggered by the recorded screening data for this session.")
    st.caption("Correctives support the training plan; they do not replace the athlete's primary or secondary development goals.")
    st.markdown("#### 3. Main Development")
    for i,x in enumerate(session["exercises"]):
        if x.system=="Corrective / Activation": continue
        slot="primary" if i<2 else "secondary"; render_exercise(a,x,week,c,slot)
    if session.get("complex"):
        cx=session["complex"]; rounds,reps,between,round_rest=complex_dose(cx,a,week,c)
        st.markdown("#### 4. Complex / Compound Athletic Block")
        names=[EXERCISES[eid].name for eid in cx.exercises if eid in EXERCISES]
        render_card(cx.method,cx.name,f"{rounds} rounds | {reps}",f"Rest {round_rest} between rounds",f"{between} between exercises","Multi-planar","Advanced / Athletic",ACCENTS.get("Plyometrics","#ec4899"))
        st.caption(" -> ".join(names))
    cond=session["conditioning"]
    st.markdown("#### 5. MetCon / Energy System Development")
    render_card("Dynamic MetCon / ESD",cond["name"],cond["work"]+" | Rest: "+cond["rest"],cond["intensity"],"Dynamic Pace","Multi-planar","Energy System / Conditioning",ACCENTS["Aerobic"])
    for station in cond["stations"]: st.markdown(f"- **{station}**")


def smart_warmup(a,session):
    # Preserve the existing concrete warm-up engine, then add one goal-specific
    # primer so two materially different session architectures do not collapse
    # into the same generic warm-up.
    base=[]
    names=" ".join(x.name.lower() for x in session.get("exercises",[]))
    if "Ergometers (AirBike/Rower/SkiErg)" in a.equipment: base.append("AirBike / Rower / SkiErg - 3-5 min easy, RPE 3-4")
    else: base.append("Fast March / Easy Jog - 3-5 min easy")
    if a.primary_goal in ["Strength","Max Strength"]:
        base.append("Empty-Bar / Light-Load Movement Rehearsal - 2 x 5 reps")
        base.append("Ramp-Up Sets - 2-3 progressive sets before the first main lift")
    elif a.primary_goal in ["Speed","Power"]:
        base.append("A-Skip + Ankling - 2 x 10 m")
        base.append("Build-Up Run - 2 x 10-20 m, progressive")
    elif a.primary_goal in ["Agility","Sport Performance"]:
        base.append("Lateral Shuffle - 2 x 5 m/side")
        base.append("Deceleration Stick - 2 x 2/side")
    elif a.primary_goal=="Hypertrophy":
        base.append("Controlled Squat-to-Stand - 1 x 6 reps")
        base.append("Light Rehearsal Set - 1 x 8 reps")
    else:
        base.append("Squat-to-Stand - 1 x 6 reps")
        base.append("Single-Leg Balance Reach - 1 x 3/side")
    findings=_v12_findings(a)
    ftext=" ".join(f[2] for f in findings)
    if any(k in ftext for k in ["scapular","shoulder","rounded shoulders","thoracic"]):
        base.append("Serratus Wall Slide - 2 x 6 reps")
        base.append("Prone Y Raise - 1 x 6-8 reps, light")
    if any(k in ftext for k in ["pelvic","hip","ankle","knee"]):
        base.append("90/90 Hip Flow - 1 x 5/side")
        base.append("Ankle Rock + Knee Drive - 1 x 6/side")
    # Keep the warm-up concise.
    out=[]
    for item in base:
        if item not in out: out.append(item)
    return out[:9]
...
def _v12_session_for(a,month,week,day,months):
    program,engine=build_program(a,months)
    return program[month][week][day-1],engine


def save_exercise_feedback(a,session,rows,session_rpe,pain_after,notes,component_rows=None,performance_change=0,duration_min=60):
    stamp=datetime.now().isoformat(timespec="seconds")
    for row in rows:
        row=dict(row); row.update({"timestamp":stamp,"athlete":a.name,"sport":a.sport,"month":session["month"],"week":session["week"],"day":session["day"],"session_rpe":float(session_rpe),"pain":int(pain_after)})
        st.session_state.exercise_feedback_log.append(row)
    if component_rows:
        for row in component_rows:
            row=dict(row); row.update({"timestamp":stamp,"athlete":a.name,"sport":a.sport,"month":session["month"],"week":session["week"],"day":session["day"],"session_rpe":float(session_rpe),"pain":int(pain_after)})
            st.session_state.session_component_feedback_log.append(row)
    summary={"timestamp":stamp,"athlete":a.name,"sport":a.sport,"month":session["month"],"week":session["week"],"day":session["day"],"session_rpe":float(session_rpe),"pain_after":int(pain_after),"performance_change":int(performance_change),"duration_min":int(duration_min),"components_recorded":len(component_rows or []),"notes":notes}
    st.session_state.session_feedback_log.append(summary)
    st.session_state.last_feedback=summary
...
if "plan_months" not in st.session_state: st.session_state.plan_months=3
plan_months=st.sidebar.select_slider("Macrocycle Horizon",options=[1,2,3,4,5,6],key="plan_months",format_func=lambda x:f"{x}-Month Block")
...
if page=="1. Athlete Profile":
    ...
    with c2:
        setv("height_cm",state_number_input("Height (cm)","height_cm",120.0,230.0,float(st.session_state.height_cm),step=0.5))
        setv("weight_kg",state_number_input("Current Weight (kg)","weight_kg",30.0,250.0,float(st.session_state.weight_kg),step=0.5))
        setv("target_weight_kg",state_number_input("Target Weight (kg)","target_weight_kg",30.0,250.0,float(st.session_state.target_weight_kg),step=0.5))
        setv("training_years",state_number_input("S&C Experience (years)","training_years",0.0,30.0,float(st.session_state.training_years),step=0.5))
    with c3:
        setv("gym_days_available",state_slider("Gym Days / Week","gym_days_available",1,7,int(st.session_state.gym_days_available)))
        setv("session_minutes",state_slider("Session Duration (min)","session_minutes",30,150,int(st.session_state.session_minutes),5))
        setv("notes",st.text_area("Coach Notes",st.session_state.notes,height=100))
    a=athlete(); ctx=anthropometric_context(a); x,y,z,w=st.columns(4)
    x.metric("Training Level",training_level(a.training_years)); y.metric("BMI Context",f"{ctx['bmi']:.1f}"); z.metric("Current Weight",f"{a.weight_kg:.1f} kg"); w.metric("Target",f"{a.target_weight_kg:.1f} kg")
    if a.primary_goal in ["Weight Loss","Weight Gain"]:
        direction="loss" if a.primary_goal=="Weight Loss" else "gain"
        st.info(f"Body-mass goal active: {direction} toward {a.target_weight_kg:.1f} kg. Height and current mass also feed normalized performance, relative strength and training-volume decisions.")
    ...
    setv("secondary_positions",multi_choice("Secondary Positions / Events",secondary_options,st.session_state.secondary_positions,"secondary_positions",columns=2))
    c1,c2=st.columns(2)
    with c1: setv("primary_goal",st.selectbox("Primary Goal",GOALS,index=GOALS.index(st.session_state.primary_goal)))
    with c2: setv("secondary_goals",multi_choice("Secondary Development Targets",SECONDARY_OPTIONS,st.session_state.secondary_goals,"secondary_goals",columns=2,help_text="Select as many targets as needed. The choices remain visible on mobile."))
...
elif page=="3. Health / Load / Readiness":
    ...
    with c1:
        ...
        setv("pain_score",state_slider("Pain score (0-10)","pain_score",0,10,int(st.session_state.pain_score)))
        ...
        setv("injuries",multi_choice("Current / recent injury history",injury_opts,st.session_state.injuries,"injuries",columns=2))
    with c2:
        setv("sleep_hours",state_slider("Sleep (hours)","sleep_hours",3.0,12.0,float(st.session_state.sleep_hours),0.5))
        setv("readiness",state_slider("Subjective Readiness","readiness",0,100,int(st.session_state.readiness)))
        setv("stress",state_slider("Stress","stress",0,10,int(st.session_state.stress)))
        setv("soreness",state_slider("Soreness","soreness",0,10,int(st.session_state.soreness)))
    with c3:
        setv("team_days",state_slider("Sport / Club Sessions per Week","team_days",0,14,int(st.session_state.team_days)))
        setv("team_minutes",st.number_input("Average Club Session Minutes",0,300,int(st.session_state.team_minutes),5))
        setv("weekly_sport_rpe",state_slider("Typical Sport Session RPE","weekly_sport_rpe",1.0,10.0,float(st.session_state.weekly_sport_rpe),0.5))
    ...
    with tabs[4]:
        ...
        instructions={
            "Cervical Flexion":"Feet together and knees straight. Keep the trunk still. Slowly tuck the chin and flex the neck forward as far as comfortable. Stop if pain appears. Observe range, smoothness and any trunk movement.",
            "Cervical Extension":"Feet together and knees straight. Keep the chest and trunk quiet. Slowly look upward by extending the neck. Do not create the movement by leaning the whole trunk backward. Observe range, pain and substitution.",
            "Cervical Rotation":"Feet together, shoulders relaxed and trunk facing forward. Turn the head fully to the left, return to neutral, then fully to the right. Keep the shoulders from rotating with the head. Compare sides and record pain or restriction.",
            "UE Pattern 1":"Stand tall. Reach one arm up and over the shoulder while the other arm reaches behind the back. Perform the pattern on both sides. Keep the trunk upright and avoid twisting to create range. Record the side that limits, pain and compensation.",
            "UE Pattern 2":"Stand tall and reverse the arm positions from UE Pattern 1: the opposite arm reaches overhead while the other reaches behind the back. Test both sides. Compare shoulder motion and note trunk rotation, rib flare or pain.",
            "Multi-Segmental Flexion":"Stand with feet together and knees straight. Bend forward by flexing through the whole spine and hips, allowing the arms to hang toward the floor. Do not force the range. Observe whether the movement is continuous and where motion appears limited or painful.",
            "Multi-Segmental Extension":"Stand with feet together and arms overhead. Extend through the spine and hips while keeping the knees controlled. Do not simply push the hips forward or bend the knees to create range. Observe the full chain, pain and compensation.",
            "Multi-Segmental Rotation":"Stand with feet together and arms across the chest. Rotate the whole body to the left, return to center, then rotate right. Keep the feet planted. Compare sides and identify the region that appears to limit or compensate.",
            "Single Leg Stance":"Test one leg at a time with the athlete standing tall and the opposite foot off the floor. Allow the athlete to stabilize without holding onto support. Observe foot/ankle strategy, pelvic control, trunk sway and loss of balance; repeat on the other side.",
            "Deep Squat":"Use the athlete's normal squat stance. Reach the arms overhead and descend under control while keeping the heels grounded. Observe depth, knee/foot alignment, trunk position, shoulder position, pain and any side-to-side compensation."
        }
    ...
    with tabs[5]:
        ...
        groups={
            "Cervical":{"Cervical Flexion":(0.0,90.0),"Cervical Extension":(0.0,90.0),"Cervical Rotation":(0.0,120.0)},
            "Shoulder":{"Shoulder Flexion":(0.0,220.0),"Shoulder Abduction":(0.0,220.0),"Shoulder IR":(0.0,120.0),"Shoulder ER":(0.0,120.0)},
            "Thoracic":{"Thoracic Rotation":(0.0,90.0)},
            "Hip":{"Hip Flexion":(0.0,160.0),"Hip Extension":(0.0,60.0),"Hip Abduction":(0.0,90.0),"Hip Adduction":(0.0,60.0),"Hip IR":(0.0,90.0),"Hip ER":(0.0,90.0)},
            "Knee":{"Knee Flexion":(0.0,160.0)},
            "Ankle":{"Ankle Dorsiflexion":(0.0,60.0),"Ankle Plantarflexion":(0.0,90.0),"Ankle Inversion":(0.0,60.0),"Ankle Eversion":(0.0,45.0)},
            "Wrist":{"Wrist Flexion":(0.0,100.0),"Wrist Extension":(0.0,100.0)},
        }
        for group,items in groups.items():
            ...
            for name,(mn,mx) in items.items():
                ...
                with c2: rom.setdefault(name,{"L":75.0,"R":75.0}); rom[name]["L"]=st.number_input("Left",min_value=float(mn),max_value=float(mx),value=float(rom[name]["L"]),step=0.5,key=f"rom_{name}_L")
                with c3: rom[name]["R"]=st.number_input("Right",min_value=float(mn),max_value=float(mx),value=float(rom[name]["R"]),step=0.5,key=f"rom_{name}_R")
    ...
    stability_groups={
        "Static / unilateral":{"Single-Leg Stance L":(0.0,180.0),"Single-Leg Stance R":(0.0,180.0)},
        "Dynamic balance":{"Y Balance ANT L":(0.0,150.0),"Y Balance ANT R":(0.0,150.0),"Y Balance PM L":(0.0,180.0),"Y Balance PM R":(0.0,180.0),"Y Balance PL L":(0.0,180.0),"Y Balance PL R":(0.0,180.0)},
        "Movement control":{"Landing Control":(0.0,100.0),"Trunk Stability":(0.0,100.0),"Single-Leg Squat Control L":(0.0,100.0),"Single-Leg Squat Control R":(0.0,100.0)},
    }
    for group,items in stability_groups.items():
        st.markdown(f"### {group}")
        cols=st.columns(2)
        for i,(n,(mn,mx)) in enumerate(items.items()):
            with cols[i%2]: t[n]=st.number_input(n,min_value=float(mn),max_value=float(mx),value=float(t.get(n,(30.0 if 'Stance' in n else 80.0))),step=0.5,key=f"stab_{n}")
    ...
    with c3:
        ...
        st.number_input("Deadlift 1RM (kg)",20.0,450.0,float(st.session_state.deadlift_1rm),1.0,key="perf_deadlift")
        setv("deadlift_1rm",st.session_state.perf_deadlift)
...
elif page=="9. Equipment & Club Load":
    st.markdown('<div class="banner-header">09 | Equipment & Club Load</div>',unsafe_allow_html=True)
    st.markdown("### Club / Team Load")
    setv("club_days",multi_choice("Club / Team Training Days",DAYS if "DAYS" in globals() else ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],st.session_state.get("club_days",[]),"club_days",columns=2))
    setv("team_minutes",st.number_input("Average Club Session Minutes",0,300,int(st.session_state.team_minutes),5))
    setv("competition_days",state_slider("Competitions / Matches per Week","competition_days",0,4,int(st.session_state.competition_days)))
    ...
    st.markdown("### Equipment Constraints")
    setv("equipment",multi_choice("Available Equipment",EQUIPMENT,st.session_state.equipment,"equipment",columns=2,help_text="Select every implement the athlete can actually use. Selection is a hard programming constraint."))
...
elif page=="11. Session Follow-Up":
    st.markdown('<div class="banner-header">11 | Full Session Follow-Up -> Next Session Adaptation</div>',unsafe_allow_html=True)
    a=athlete(); program,engine=build_program(a,plan_months)
    m=st.selectbox("Month",list(range(1,plan_months+1)),index=max(0,min(plan_months-1,int(st.session_state.selected_session.get("month",1))-1)),key="fu_month")
    w=st.selectbox("Week",[1,2,3,4],index=max(0,min(3,int(st.session_state.selected_session.get("week",1))-1)),key="fu_week")
    d=st.selectbox("Day",list(range(1,a.gym_days_available+1)),index=max(0,min(a.gym_days_available-1,int(st.session_state.selected_session.get("day",1))-1)),key="fu_day")
    st.session_state.selected_session={"month":m,"week":w,"day":d}
    session=program[m][w][d-1]
    st.markdown(f"<div class='goal-card'><b>Recording: Month {m} | Week {w} | Day {d}</b><br>Record the entire session: warm-up, correctives, main work, complex/athletic block and MetCon. Resistance still gets the detailed load/reps/RPE tracking used for adaptive loading.</div>",unsafe_allow_html=True)
    component_rows=[]; resistance_rows=[]

    st.markdown("### 1. Smart Warm-up")
    for i,item in enumerate(smart_warmup(a,session)):
        c1,c2,c3=st.columns([4,1,2])
        with c1: st.write(item)
        with c2: done=st.checkbox("Done",value=True,key=f"fu_warm_done_{m}_{w}_{d}_{i}")
        with c3: rpe=st.slider("RPE",1.0,10.0,3.0,0.5,key=f"fu_warm_rpe_{m}_{w}_{d}_{i}")
        component_rows.append({"component_type":"Warm-up","component_name":item,"status":"Completed" if done else "Skipped","rpe":rpe,"performance":"Expected" if done else "Skipped"})

    correctives=screening_correctives(a,session,limit=3)
    st.markdown("### 2. Screening Correctives / Movement Control")
    if correctives:
        for i,(x,side,reason) in enumerate(correctives):
            c1,c2,c3,c4=st.columns([3,1,1,2])
            with c1: st.write(f"{x.name} | Side: {'Left' if side=='L' else 'Right' if side=='R' else 'Both'}")
            with c2: done=st.checkbox("Done",value=True,key=f"fu_cor_done_{m}_{w}_{d}_{i}")
            with c3: rpe=st.slider("RPE",1.0,10.0,5.0,0.5,key=f"fu_cor_rpe_{m}_{w}_{d}_{i}")
            with c4: perf=st.selectbox("Performance",["Easy","Expected","Hard","Modified","Skipped"],index=1,key=f"fu_cor_perf_{m}_{w}_{d}_{i}")
            component_rows.append({"component_type":"Corrective","component_name":x.name,"exercise_id":x.id,"side":side,"screening_link":reason,"status":"Completed" if done else "Skipped","rpe":rpe,"performance":perf})
    else:
        st.caption("No targeted corrective was triggered by the recorded screening data.")

    st.markdown("### 3. Main Development")
    for i,x in enumerate(session.get("exercises",[])):
        slot="primary" if i<2 else "secondary"
        if x.system=="Resistance":
            dose=v7_resistance_dose(a,w,slot); planned_load,_=adaptive_exercise_load(a,x,w,slot)
            st.markdown(f"#### {x.name}")
            c1,c2,c3,c4=st.columns(4)
            with c1: actual_load=st.number_input("Actual load (kg)",0.0,500.0,float(planned_load),0.5,key=f"fu_load_{m}_{w}_{d}_{x.id}")
            with c2: reps_done=st.number_input("Reps completed",0,int(max(30,dose["reps"]*2)),int(dose["reps"]),1,key=f"fu_reps_{m}_{w}_{d}_{x.id}")
            with c3: ex_rpe=st.slider("Exercise RPE",1.0,10.0,7.0,0.5,key=f"fu_rpe_{m}_{w}_{d}_{x.id}")
            with c4: tech=st.selectbox("Technique",["Excellent","Good","Acceptable","Poor"],index=1,key=f"fu_tech_{m}_{w}_{d}_{x.id}")
            perf=st.selectbox("Performance",["Easy","Expected","Hard","Failed / could not complete"],index=1,key=f"fu_perf_{m}_{w}_{d}_{x.id}")
            resistance_rows.append({"exercise_id":x.id,"exercise_name":x.name,"component_type":"Resistance","planned_load":planned_load,"planned_reps":dose["reps"],"actual_load":actual_load,"reps_completed":reps_done,"exercise_rpe":ex_rpe,"technique":tech,"performance":perf})
        else:
            st.markdown(f"#### {x.name}")
            c1,c2,c3=st.columns(3)
            with c1: done=st.checkbox("Completed",value=True,key=f"fu_main_done_{m}_{w}_{d}_{x.id}")
            with c2: ex_rpe=st.slider("RPE",1.0,10.0,6.0,0.5,key=f"fu_main_rpe_{m}_{w}_{d}_{x.id}")
            with c3: perf=st.selectbox("Performance",["Easy","Expected","Hard","Modified","Skipped"],index=1,key=f"fu_main_perf_{m}_{w}_{d}_{x.id}")
            component_rows.append({"component_type":x.system,"component_name":x.name,"exercise_id":x.id,"status":"Completed" if done else "Skipped","rpe":ex_rpe,"performance":perf})

    if session.get("complex"):
        cx=session["complex"]; names=[EXERCISES[eid].name for eid in cx.exercises if eid in EXERCISES]
        st.markdown("### 4. Complex / Compound Athletic Block")
        c1,c2,c3,c4=st.columns(4)
        with c1: st.write(cx.name)
        with c2: completed_rounds=st.number_input("Rounds completed",0,20,3,1,key=f"fu_cx_rounds_{m}_{w}_{d}")
        with c3: cx_rpe=st.slider("Block RPE",1.0,10.0,7.0,0.5,key=f"fu_cx_rpe_{m}_{w}_{d}")
        with c4: cx_perf=st.selectbox("Performance",["Easy","Expected","Hard","Modified","Skipped"],index=1,key=f"fu_cx_perf_{m}_{w}_{d}")
        st.caption(" -> ".join(names))
        component_rows.append({"component_type":"Complex","component_name":cx.name,"status":"Completed" if completed_rounds>0 else "Skipped","rounds_completed":completed_rounds,"rpe":cx_rpe,"performance":cx_perf})

    cond=session["conditioning"]
    st.markdown("### 5. MetCon / Energy System Development")
    st.caption(cond["name"]+" | "+cond["work"]+" | Rest: "+cond["rest"])
    for i,station in enumerate(cond["stations"],1): st.markdown(f"**{i}.** {station}")
    c1,c2,c3,c4=st.columns(4)
    with c1: met_status=st.selectbox("Session completion",["Full","Partial","Modified","Skipped"],index=0,key=f"fu_met_status_{m}_{w}_{d}")
    with c2: met_rpe=st.slider("MetCon RPE",1.0,10.0,7.0,0.5,key=f"fu_met_rpe_{m}_{w}_{d}")
    with c3: met_perf=st.selectbox("Performance",["Easy","Expected","Hard","Failed / could not complete"],index=1,key=f"fu_met_perf_{m}_{w}_{d}")
    with c4: met_rounds=st.number_input("Rounds / intervals completed",0,50,2,1,key=f"fu_met_rounds_{m}_{w}_{d}")
    component_rows.append({"component_type":"MetCon","component_name":cond["name"],"status":met_status,"rpe":met_rpe,"performance":met_perf,"rounds_completed":met_rounds,"stations":cond["stations"]})

    st.markdown("### 6. Session Summary")
    c1,c2,c3=st.columns(3)
    with c1: session_rpe=st.slider("Overall Session RPE",1.0,10.0,7.0,0.5,key="fu_session_rpe")
    with c2: pain_after=st.slider("Pain During / After Session",0,10,0,key="fu_pain")
    with c3: duration=st.number_input("Actual session duration (min)",20,180,int(a.session_minutes),5,key="fu_duration")
    performance_change=st.slider("Overall performance vs expected",-2,2,0,key="fu_perf_overall")
    notes=st.text_area("Coach notes",key="fu_notes")
    if st.button("Save Session & Update Next Decisions",type="primary"):
        save_exercise_feedback(a,session,resistance_rows,session_rpe,pain_after,notes,component_rows=component_rows,performance_change=performance_change,duration_min=duration)
        log_training_session(a,session_rpe,pain_after,performance_change,duration,notes)
        st.success("Full session saved. Exercise loads, conditioning demand and future decisions will use the recorded outcome.")
    logs=st.session_state.get("exercise_feedback_log",[])
    if logs:
        st.markdown("### Recent Resistance / Exercise History")
        st.dataframe(pd.DataFrame(logs[-30:]),use_container_width=True,hide_index=True)
    comps=st.session_state.get("session_component_feedback_log",[])
    if comps:
        st.markdown("### Recent Full-Session Component History")
        st.dataframe(pd.DataFrame(comps[-30:]),use_container_width=True,hide_index=True)
    sf=st.session_state.get("session_feedback_log",[])
    if sf:
        st.markdown("### Recent Session History")
        st.dataframe(pd.DataFrame(sf[-12:]),use_container_width=True,hide_index=True)

elif page=="12. Data / Profiles":
    st.markdown('<div class="banner-header">12 | Data / Profiles</div>',unsafe_allow_html=True)
    a=athlete()
    ...
    if st.session_state.profiles: st.write(list(st.session_state.profiles.keys()))
    if st.session_state.get("exercise_feedback_log"):
        st.download_button("Download Exercise Feedback CSV",pd.DataFrame(st.session_state.exercise_feedback_log).to_csv(index=False),"athlete_iq_exercise_feedback.csv","text/csv")
    if st.session_state.get("session_component_feedback_log"):
        st.download_button("Download Full Session Feedback CSV",pd.DataFrame(st.session_state.session_component_feedback_log).to_csv(index=False),"athlete_iq_full_session_feedback.csv","text/csv")
