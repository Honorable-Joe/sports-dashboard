...
def state_slider(label, key, *args, **kwargs):
    """Render a state-backed slider without mutating the widget's own key.

    The previous implementation used the same session-state key for the widget
    and then setv() wrote to that key immediately after rendering. Streamlit
    correctly rejects that pattern with StreamlitWidgetAlreadyInstantiatedError.
    A private widget key keeps widget state and application state separate.
    """
    if key not in st.session_state:
        if "value" in kwargs:
            st.session_state[key]=kwargs["value"]
        elif len(args)>=3:
            st.session_state[key]=args[2]
        elif len(args)>=2:
            st.session_state[key]=args[1]
    widget_key=f"__aiq_widget_slider_{key}"
    return st.slider(label, *args, value=st.session_state[key], key=widget_key, **kwargs)

def state_number_input(label, key, *args, **kwargs):
    """Render a state-backed number input using a private Streamlit widget key."""
    if key not in st.session_state:
    ...
    return st.number_input(label, *args, value=st.session_state[key], key=widget_key, **kwargs)
