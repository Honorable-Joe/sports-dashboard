...
if page=="1. Athlete Profile":
    ...
    with tabs[5]:
        ...
        for group,items in groups.items():
            ...
            for name,(mn,mx) in items.items():
                ...
                with c2: rom.setdefault(name,{"L":75.0,"R":75.0}); rom[name]["L"]=st.number_input("Left",mn,mx,float(rom[name]["L"]),0.5,key=f"rom_{name}_L")
                with c3: rom[name]["R"]=st.number_input("Right",mn,mx,float(rom[name]["R"]),0.5,key=f"rom_{name}_R")
    ...
    for group,items in stability_groups.items():
        ...
        for i,(n,(mn,mx)) in enumerate(items.items()):
            with cols[i%2]: t[n]=st.number_input(n,mn,mx,float(t.get(n,(30 if 'Stance' in n else 80))),0.5,key=f"stab_{n}")
