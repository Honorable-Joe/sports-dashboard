...
spec=importlib.util.spec_from_file_location('aiq','/mnt/data/aiq_v12/app.py'); aiq=importlib.util.module_from_spec(spec); spec.loader.exec_module(aiq)
...
src=open('/mnt/data/aiq_v12/app.py',encoding='utf-8').read();
for bad in ['Ã','â','ð','�']:
    assert bad not in src, f'Mojibake marker {bad} found'
print('EXTENDED V11 TESTS PASS')

# V12 screening: posture asymmetry triggers targeted correctives
x=base(); x.posture_posterior={"Shoulder height": "Right Elevated || Moderate ||"}; sess={"exercises":[]}
cs=aiq.screening_correctives(x,sess,limit=3); assert cs, "Posture asymmetry did not trigger correctives"
# V12 mobility: directional hip extension restriction triggers corrective
x=base(); x.mobility_rom["Hip Extension"]={"L":5.0,"R":15.0}; cs=aiq.screening_correctives(x,sess,limit=3); assert cs, "Hip mobility restriction did not trigger corrective"
# V12 adaptive load: an easy successful exposure increases next load
x=base(); dead=[e for e in aiq.EXERCISES.values() if "deadlift" in e.name.lower() and e.system=="Resistance"][0]
aiq.st.session_state.exercise_feedback_log=[{"athlete":x.name,"exercise_id":dead.id,"actual_load":80.0,"exercise_rpe":6.0,"reps_completed":5,"planned_reps":5,"technique":"Excellent","performance":"Easy","pain":0}]
new_load,reason=aiq.adaptive_exercise_load(x,dead,1); assert new_load>80, "Easy successful exposure did not progress load"
# V12 hard exposure should not increase
aiq.st.session_state.exercise_feedback_log=[{"athlete":x.name,"exercise_id":dead.id,"actual_load":80.0,"exercise_rpe":9.5,"reps_completed":5,"planned_reps":5,"technique":"Poor","performance":"Hard","pain":0}]
new_load,reason=aiq.adaptive_exercise_load(x,dead,1); assert new_load<=80, "Hard technical exposure incorrectly increased load"
print('V12 ADAPTIVE TESTS PASS')

# Previous bug: General Fitness must not receive combat-specific sport drills
x=base(); x.sport='General Fitness'; x.position='General'; x.equipment=aiq.EQUIPMENT.copy(); x.primary_goal='Overall Development'; p,_=aiq.build_program(x,3); ns=names(p)
for bad in ['Slip -> Roll -> Counter Shadow Sequence','Karate Shadow Kumite - Technical Flow','Shadow Kumite']:
    assert bad not in ns, f'Combat drill leaked into General Fitness: {bad}'
# Previous icon bug: source must not contain pseudo-icon placeholders
src=open('/mnt/data/aiq_v12/app.py',encoding='utf-8').read(); assert '[POWER]' not in src and '[PIN]' not in src
print('V12 PREVIOUS-BUG REGRESSION TESTS PASS')
# V12 same-week anchor re-exposure architecture: anchor lifts are allowed to recur later in the week.
x=base(); x.sport='General Fitness'; x.primary_goal='Strength'; x.gym_days_available=4; x.equipment=aiq.EQUIPMENT.copy(); p,_=aiq.build_program(x,1)
all_day=[[e.name for e in p[1][1][d]['exercises'] if e.system=='Resistance'] for d in range(4)]
# The architecture must not globally forbid a repeated anchor; verify at least one anchor repeats when available.
anchors=set(n for n in all_day[0] if any(k in n.lower() for k in ['deadlift','squat','bench','row','press','pull-up','hip thrust','romanian']))
repeats=set(all_day[3]).intersection(anchors)
assert repeats or not anchors, 'Same-week anchor re-exposure was blocked on Day 4'
print('V12 SAME-WEEK ANCHOR TEST PASS')
